...
function PhoneFrame({ children, caption }: { children: ReactNode; caption?: string }) {
  return (
    <div className="my-3">
      ...
      {caption && (
        <p className="text-[12px] text-gray-500 text-center mt-2">{caption}</p>
      )}
    </div>
  )
}
  ...
  return (
    <PageContainer>
      ...
      <Step num={1} title="納付者情報を登録">
        ...
        <Hint>
          納期の特例（半年に1回納付）を適用している場合は、トグルをONにしてください。納付書の集計単位が自動で半期になります。
        </Hint>
      </Step>

      <Step num={2} title="従業員を登録">
        <p className="text-[14px] text-gray-700 leading-relaxed mb-3">
          <b>⚙設定 → 従業員の登録</b> から、ホステス・乙欄従業員を1人ずつ追加します。
          氏名・フリガナ・<b className="text-pink-600">源氏名</b>・住所・生年月日・メモを入力できます。
        </p>
      ...
      <Step num={3} title="税理士報酬を登録">
        <p className="text-[14px] text-gray-700 leading-relaxed mb-3">
          <b>⚙設定 → 税理士報酬の登録</b> から税理士を追加します。
          税抜の報酬額と、支払月を月別チェックボックスで指定します。
        </p>
      ...
      <Step num={4} title="その日の支払を記録">
        <p className="text-[14px] text-gray-700 leading-relaxed mb-3">
          トップの <b>「毎日の給与・報酬の支払」</b> から、その日支払う人を選んで金額を入力します。
          区分に応じて源泉税と差引支払額が自動計算されます。
        </p>
        <PhoneFrame caption="毎日の給与・報酬の支払">
          ...
          <MiniCard>
            ...
            <div className="mb-1.5">
              ...
              <div className="bg-gray-100 rounded-md px-1.5 py-1 text-[9px] flex justify-between">
                <span className="truncate">山田 花子（れな）／ホステス</span>
                <span className="text-gray-400">▽</span>
              </div>
            </div>
        ...
        <Hint>
          選択肢には<b className="text-pink-600">源氏名</b>も括弧書きで表示されるので、店内で呼んでいる名前から選べます。
        </Hint>
      </Step>

      <SectionLabel>③ 月末の作業</SectionLabel>
      <Step num={5} title="納付書イメージを確認">
        <p className="text-[14px] text-gray-700 leading-relaxed mb-3">
          トップの <b>「納付書イメージを表示」</b> から、月を選んで給与所得（30203）または報酬・料金等（32319）の納付書イメージを確認します。
          紙の納付書に転記する用です。
        </p>
      ...
      <Step num={6} title="税理士提出用ファイルをダウンロード">
        <p className="text-[14px] text-gray-700 leading-relaxed mb-3">
          トップの <b>「CSV書き出し」</b> から、月を選んで <b>「税理士提出用ファイルをまとめてダウンロード」</b> をタップ。
          5つのCSVがZIPでまとまります。
        </p>
        <PhoneFrame caption="CSV書き出し">
          ...
          <div className="text-[7px] text-gray-400 mt-1">4つのCSVを1つのZIPに</div>
