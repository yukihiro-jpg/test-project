...
import { annualGiftSchema } from "@/validators/gift";

export async function GET(_req: NextRequest, { params }: { params: Promise<{ datasetId: string }> }) {
  const { datasetId } = await params;
  const gifts = await prisma.annualGift.findMany({
    where: { datasetId },
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ datasetId: string }> }) {
  ...
  if (body._update && body.id) {
    ...
    const parsed = annualGiftSchema.safeParse(updateData);
    if (!parsed.success) {
      return NextResponse.json({ error: parsed.error.flatten() }, { status: 400 });
    }
    const updated = await prisma.annualGift.update({
      where: { id },
      data: parsed.data,
    });
  ...
  const parsed = annualGiftSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json({ error: parsed.error.flatten() }, { status: 400 });
  }
  const created = await prisma.annualGift.create({
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ datasetId: string }> }) {
  ...
  await prisma.annualGift.delete({ where: { id: deleteId } });
