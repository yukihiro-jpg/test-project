...
// 添付PDFの様式をトレース。スマホで確認しながら紙の納付書に転記する用途。

import React from 'react'

export type HoshuRow = {
  code: string // e.g. '08' ホステス等
  people: number
  amount: number
  tax: number
}
...
export type HoshuSlipData = {
  reiwaYear: string // 令和年度（例: 8）
  taxOffice: string
  taxOfficeNumber: string // 3桁
  seiriNumber: string // 8桁
...
const COLS_AMOUNT = 10 // 円位まで
const COLS_PEOPLE = 6
const COLS_TAX = 10

function Digits({ value, cols, align = 'right' }: { value: string | number; cols: number; align?: 'right' | 'left' }) {
...
export default function SlipHoshu({ data }: { data: HoshuSlipData }) {
  ...
  return (
    <div className="bg-white border-2 border-gray-800 p-3 text-[11px] text-gray-800" style={{ minWidth: 720 }}>
      ...
      <div className="text-[10px] mb-1">コード表: 01 原稿料・著作権使用料等 / 05 外交員等 / 06 俳優等 / 07 芸能人事業 / <b>08 ホステス等の報酬・料金</b> / 21 契約金 / 31 賞金 / 41 公的年金等</div>

      <div className="border border-gray-700 mb-2">
        <div className="flex bg-gray-100 text-center font-semibold">
          <div className="w-16 border-r border-gray-700 py-1">区分</div>
          <div className="border-r border-gray-700 py-1" style={{ width: COLS_PEOPLE * 24 + 24 }}>人員</div>
          <div className="border-r border-gray-700 py-1" style={{ width: COLS_AMOUNT * 24 + 24 }}>支払額</div>
          <div className="py-1" style={{ width: COLS_TAX * 24 + 24 }}>税額</div>
        </div>
        {rows.map((r, i) => (
          <div key={i} className="flex border-t border-gray-700">
            <div className="w-16 border-r border-gray-700 flex justify-center items-center">
        ...
        <div>
          ...
          <div className="border border-gray-700">
            <div className="flex border-b border-gray-700">
              ...
              <div className="p-1"><Digits value={data.honzei || ''} cols={9} /></div>
            </div>
            <div className="flex border-b border-gray-700">
              <div className="w-12 px-1 py-1 border-r border-gray-700">延滞税</div>
              <div className="p-1"><Digits value={data.entaizei || ''} cols={9} /></div>
            </div>
            <div className="flex">
              <div className="w-12 px-1 py-1 border-r border-gray-700 font-bold">合計額</div>
              <div className="p-1"><Digits value={data.goukei ? `¥${data.goukei}` : ''} cols={9} /></div>
            </div>
          </div>
