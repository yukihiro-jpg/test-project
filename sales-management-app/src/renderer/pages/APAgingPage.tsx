        ...
        <div>
          ...
          <table className="w-full text-sm border mb-4">
            <thead className="bg-gray-100"><tr>
              <th className="px-2 py-1 text-left">仕入先</th><th>合計</th>
              <th>当月</th><th>1-30日</th><th>31-60日</th><th>61-90日</th><th>90日超</th>
            </tr></thead>
            <tbody>
              {data.suppliers.map((s: any) => (
                <tr key={s.supplier_id} className="border-t">
                  <td className="px-2">{s.supplier_name}</td>
                  <td className="text-right px-2">{fmt(s.total)}</td>
          ...
          <table className="w-full text-sm border">
            <thead className="bg-gray-100"><tr>
              <th>請求書番号</th><th>仕入先</th><th>発行日</th><th>支払期日</th><th>請求額</th><th>支払済</th><th>残高</th><th>区分</th>
            </tr></thead>
            <tbody>
              {data.rows.map((r: any) => (
                <tr key={r.invoice_id} className="border-t">
                  <td className="px-2">{r.invoice_no}</td>
                  <td className="px-2">{r.supplier_name}</td>
