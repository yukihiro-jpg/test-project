...
REM 顧問先名を取得して表示
echo [3/6] 設定内容を確認中...
for /f "usebackq delims=" %%i in (`powershell -NoProfile -Command "[Console]::OutputEncoding = [System.Text.Encoding]::UTF8; (Get-Content '%APP_DIR%\config.json' -Raw -Encoding UTF8 | ConvertFrom-Json).client_name"`) do set CLIENT_NAME=%%i
...
powershell -NoProfile -Command "[Console]::OutputEncoding = [System.Text.Encoding]::UTF8; $config = Get-Content '%APP_DIR%\config.json' -Raw -Encoding UTF8 | ConvertFrom-Json; foreach ($pair in $config.sync_pairs) { $folder = Join-Path '%LOCAL_FOLDER%' $pair.local_folder; if (-not (Test-Path $folder)) { New-Item -ItemType Directory -Path $folder -Force | Out-Null; Write-Host ('  作成: ' + $pair.local_folder) } else { Write-Host ('  既存: ' + $pair.local_folder) } }"
