...
// 解析対象の書類種別
const ANALYZABLE_TYPES = ['レシート・領収書', 'クレジットカード利用明細書', '通帳', '売上請求書', '仕入請求書', '賃貸送金明細'];
...
JSON配列のみを返してください。説明文は不要です。`;
    ...
    case '賃貸送金明細':
      return `この画像は不動産管理会社が発行した賃貸送金通知書（送金のご案内・賃貸管理報告書）です。
物件ごとの収入と支出と振込額をJSON配列で抽出してください。

書類の構造:
- 物件単位で「収入の部」（賃料・礼金・更新料・共益費・駐車場・償却・その他）と
  「支出の部」（管理料・清掃料・鍵交換・振替手数料・リフォーム・修繕費等）が並んでいる
- 各物件ごとに送金額（振込額）が記載されている
- 1つの書類に複数物件が含まれることが多い

抽出ルール:
- 1物件 = 1JSONオブジェクトとして配列にしてください
- "振込額" は その物件の「送金額」「振込額」「差引送金額」など、最終的に振り込まれる金額（税込ベース）
- "収入額" は収入の部の小計（税抜）。複数費目の税抜合計
- "収入消費税" は収入の部の消費税合計
- "手数料" は支出の部の小計（税込ベース。管理料・清掃料・振替手数料等の合計、税込）
- 賃料の項目ごとの明細は無視して、サマリーのみ抽出

各オブジェクトのキー:
- "対象月": "YYYY/MM"形式 または "YYYY年MM月分"
- "送金日": "YYYY/MM/DD"形式（記載があれば。なければ空文字""）
- "送金元": 不動産管理会社名（例: 一誠商事株式会社）
- "物件名": 物件名（例: メゾン・キクチ東石岡）
- "振込額": 振込・送金された金額（数値、税込）
- "収入額": 収入の部 税抜合計（数値）
- "収入消費税": 収入の部 消費税合計（数値）
- "手数料": 支出の部の合計（数値、税込）
- "備考": 補足情報、なければ空文字""
...
function writeAnalysisResult(clientSheet, docType, rows, bankName, accountNumber, userName, sourceFileName) {
  ...
  switch (docType) {
    ...
    case '賃貸送金明細':
      sheet = clientSheet.getSheetByName('賃貸送金明細');
      if (!sheet) {
...
- "賃貸送金明細": 不動産管理会社が物件オーナー（賃貸人）に賃料収入を送金する際の通知書。「送金のご案内」「送金通知書」「賃貸管理報告書」等のタイトル、または収入の部（賃料・礼金・更新料・共益費・駐車場等）と支出の部（管理料・清掃料・振替手数料等）が表で示され、差額として送金額・振込額が記載されている書類。
- "その他": 上記のいずれにも該当しない書類

判定のポイント:
- 請求書の場合、「${clientName}」またはそれに類する名称が請求元（発行者）に記載されていれば「売上請求書」
- 「${clientName}」またはそれに類する名称が宛先（請求先・お客様名）に記載されていれば「仕入請求書」
- 賃貸送金明細の特徴: 「物件名」「家賃」「管理料」「送金額」「収入の部」「支出の部」等のキーワード、または不動産管理会社（一誠商事、エイブル、ミニミニ等）からの月次送金通知。${clientName}が物件オーナー側の宛先になっている場合は「賃貸送金明細」を優先。
